...
## 更新摘要
**已做更改**
- 在“核心组件”和“详细组件分析”中新增了关于前端Logto认证配置功能的说明
- 更新了“配置与凭据来源”部分，以反映Logto配置项（Issuer URL、JWKS URI、Audience、Scopes）现在可通过前端界面进行设置
- 增加了对`AuthSettingsSection.jsx`和`config_manager.py`中相关实现的引用
- 更新了架构总览序列图以体现配置流变化
...
- **Logto 认证配置**：用户可通过前端界面配置 Issuer URL、JWKS URI、Audience 和 Required Scopes。
...
- [useAuth.js](file://frontend/src/hooks/useAuth.js#L1-L30)
- [api.js](file://frontend/src/services/api.js#L1-L403)
- [AuthSettingsSection.jsx](file://frontend/src/components/Settings/AuthSettingsSection.jsx#L1-L88)

## 架构总览
后端采用 FastAPI 依赖注入实现端点保护，前端通过 Logto 获取访问令牌并自动附加到请求头。JWT 验证链路从 HTTP 头部提取 Bearer 令牌，解析头部 kid，从 JWKS 获取匹配密钥，再进行签名验证与作用域校验。凭证存储采用数据库加密，前端提供遮罩显示以降低泄露风险。**新增的 Logto 认证配置功能允许用户通过前端界面动态设置认证参数，这些配置优先从数据库读取，环境变量作为后备**。
...
Auth-->>API : "返回用户声明"
API-->>FE : "受保护资源响应"
Note over Cfg : "Logto配置从数据库读取，环境变量为后备"
```
...
- **前端可配置**：通过 `AuthSettingsSection.jsx` 组件，用户可在前端界面直接配置 Logto 的 Issuer URL、JWKS URI、Audience 和 Required Scopes，这些值将被加密存储在数据库中。
...
- **前端配置依赖**：`AuthSettingsSection.jsx` 依赖 `config_manager.py` 的 `get_logto_config` 和 `update_credentials` 接口来读取和保存 Logto 配置。
...
FE_Config["frontend/AuthSettingsSection.jsx"] --> CFG
```
...
- **配置灵活性**：Logto 认证参数的前端可配置性提高了系统的灵活性和可维护性，同时通过数据库优先策略确保了配置的集中管理。
...
- Logto 配置更新不生效
  - 确认配置已通过前端界面保存至数据库。
  - 检查 `config_manager.py` 的 `get_logto_config` 是否正确从数据库读取。
  - 参考：[config_manager.py](file://backend/src/config/config_manager.py#L134-L147)，[AuthSettingsSection.jsx](file://frontend/src/components/Settings/AuthSettingsSection.jsx#L1-L88)
...
- [config_manager.py](file://backend/src/config/config_manager.py#L134-L147)
- [AuthSettingsSection.jsx](file://frontend/src/components/Settings/AuthSettingsSection.jsx#L1-L88)

## 结论
- 当前系统在认证方面采用标准 JWT 流程，具备良好的可扩展性与安全性基线。
- 凭证加密与遮罩显示形成闭环，配合 DB 优先的配置管理，降低了泄露与误配置风险。
- 新增的前端 Logto 认证配置功能显著提升了系统的可维护性和灵活性。
...
- **Logto 配置管理**：通过前端界面配置的 Logto 参数（Issuer URL、JWKS URI 等）将覆盖环境变量，确保这些值的准确性和安全性至关重要。
